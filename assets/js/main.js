        ...
        for(var j = 0; j< inputArray[n].length; j++){
            if($(inputArray[n][j]).val() != ""){
                ...
                inputArray[inputData[0]].forEach(function(element, key){
                    if($(inp).val() !== "" && $(element).val() === $(inp).val() && $(element).attr("data-input") !== $(inp).attr("data-input")){
                ...
                inputBox.children().each(function(index, element){
                    ...
                    if($(inp).val() !== "" && element.value === $(inp).val() && $(element).index() !== $(inp).index()){
                        $(inp).removeClass("bg-light");
                        $(inp).addClass("bg-warning");
                        $(element).addClass("bg-warning");
                        rest = false;
                ...
                if(!rest && !$(inp).prop("disabled")){
                    console.log($(inp).prop("disabled"));
                    inputsTrue--;
                }
            } else {
   ...
   // console.log((inputsTrue*100/totalInputs).toFixed(2))
    ...
    switch (count){
        ...
        case 7:
            ...
            break;
        default:
            return {status: false, user:name, level:$("h1").eq(0).text(), score:totalScore+"%", message: "Your sudoku is empty..."};
            break;
    }
}
